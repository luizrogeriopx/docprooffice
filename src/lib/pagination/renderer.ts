...
import type { PaginationBreakSpec } from "./types";

export const paginationBreaksKey = new PluginKey<DecorationSet>("docproPaginationBreaks");

export function paginationBreaksSignature(breaks: PaginationBreakSpec[]): string {
  return breaks
    .map(
      (item) =>
        `${item.pos}:${Math.round(item.height)}:${item.tag || "div"}:${
          item.repeatedTableHeaderHtml ? "h" : "nh"
        }`
    )
    .join("|");
}
...
export const PaginationBreaksPlugin = new Plugin({
        ...
        const widgets = meta.map((item) =>
          Decoration.widget(
            ...
            () => {
              ...
              if (tag === "tr") {
                if (item.repeatedTableHeaderHtml && item.tableColsCount) {
